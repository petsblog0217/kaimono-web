...
## 3. 主な機能
- ホームに全店を縦に並べて表示＋上部タブ。商品タップで買い済み→「在庫へ」自動補充。家の在庫ページ（増減・カゴへ・削除）。
- ひらがな先頭一致の候補から追加（辞書DB）。新規はアイコン選択可（色は店の色に統一）。
- 設定：お店の追加/削除/色/カテゴリ編集。商品（辞書）の管理＝検索＋お店ごと折りたたみ＋2列タイル、編集/削除/追加。
- 自動並び替え：辞書も買い物リストもカテゴリ＋ふりがな50音順（買い物リストは買い済みを下に維持）。
- 買い物リストのタイル：タップ＝買い済みトグル、長押し（0.5秒）＝ミニメニュー（数量 −＋・リストから削除）。`attachLongPress`＋`itemSheet`。
